...
** update
now we are going to use dependency injection to inject the multiple beans(implementations) from context with @Qualifier annotation

*changes*

3- look add proxies package added new class implementing the same interface
** 4- use @Qualifier annotation to name a bean and specify it for DI to inject it from context